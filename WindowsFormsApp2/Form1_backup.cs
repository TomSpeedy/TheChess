...
namespace WindowsFormsApp2
{
    ...
    public partial class Intro : Form  //vytvorenie a obsluha tlacitok, pomocou ktorych sa vybera typ hry
    {
        ...
        public void InitializeIntro(int a)
        {
            this.Height = 500;
            this.Width = 700;
            this.Name = "Welcome";
        ...
        public void InitTextBox1(int a)
        {
        ...
        public void InitTextBox2(int a)
        {
    ...
    public partial class TheChess : Form
    {
        ...
        public void TransClicked(object sender, EventArgs e)
        {
            ...
            if ((FindCheck(ref chessboard, 6 * turn) == true)
                && (CountMoves(chessboard) !=0) && ((turn == player_turn) || multi_player == true))
            { 
                const string message = "You are in check";
                MessageBox.Show(message, "Check", MessageBoxButtons.OK);
            }
        ...
        public int CountMoves(ChessPiece[,] chess_state)
        {
            ...
            {
                if ((piece.exist == true) && (piece.id * turn > 0))
                {
                    UpdatePossibleMoves(ref chess_state, piece.position_x, piece.position_y);
                    DeleteCheckMoves(ref chess_state, piece.position_x, piece.position_y);
                    sum = sum + chess_state[piece.position_x, piece.position_y].possible_moves.Count;
                }
            }
            */
            for (int i = 0; i < 8; i++)
        ...
        public int Max(ref ChessPiece[,] state, int depth, int final_depth, int alpha, int beta) //simuluje tah hraca
        {
            ...
            if ((first_call == true) && final_depth >= 3)
            {
                ...
                turn = -1 * turn;
                int value;
                number_of_moves = CountMoves(state);
                ...
                {
                    value = Judge.Evaluate(state, -1*turn, number_of_moves, turn);
                }
                if (value > max)
                {
                    max = value;
                    ...
                    if ((state[i, j].id * turn > 0) && (state[i, j].exist == true))
                    {
                        ...
                        foreach (Coordinates possible_move in possible_moves)
                        {
                            ...
                            else
                            {
                                ...
                                if ((depth != final_depth) && (CountMoves(state) != 0))
                                {
                    ...
                    if ((state[i, j].id * turn > 0) && (state[i, j].exist == true))
                    {
                        ...
                        foreach (Coordinates possible_move in possible_moves)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    value = Judge.Evaluate(state, turn, number_of_moves, turn);
                                }
                                if (beta > value)
                                {
                                    beta = value;
        ...
        public TheChess() //prvotna inicializacia hlavneho formulara
        {
            ...
            intro.InitializeIntro(1);
            intro.Show();


        }
        public void InitStart(int a)
        {
